...
function AppBarLinks({user, logout}) {
  ...
  return (
    <>
      ...
      {!user && (
        <>
          <Link to={`/register?from=${encodeURIComponent(currentPath)}`}>
          ...
          <AppBar position="static" className='AppBar'>
            <Container>
              <Toolbar disableGutters={true}>
                <Link to="/"><Typography variant="h6" component="div">Descode</Typography></Link>
                <Link to="/reviews"><Typography>Reviews</Typography></Link>
                <Box sx={{flexGrow:1}}></Box>
                {/* Use the AppBarLinks component so we have access to useLocation */}
